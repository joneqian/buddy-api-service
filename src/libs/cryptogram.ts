 ...
 * @param ciphertext
 */
export function aes256gcmDecrypt(key: string, nonce: string, associatedData: string, ciphertext: string): string {
...
export function apiCipher(data: any, key: string): string {
  ...
  return cryptoJS.AES.encrypt(typeof data === 'object' ? JSON.stringify(data) : data.toString(), key).toString();
}
